{
  ...
  cifsShares = [
    rec {
      ...
      options = user:
        (cifsCommonOptions user) ++ ["credentials=${credentials user}"];
    }
    rec {
      name = "home";
      what = "//nas1/home";
...
in {
  ...
  fileSystems =
    lib.mkIf
    (builtins.elem config.networking.hostName ["offnix" "devnix" "magnix"])
    (builtins.listToAttrs (builtins.concatLists (map (share:
      map (user: {
  ...
  systemd.tmpfiles.rules =
    ...
    (["d /etc/nixos/secrets 0700 root root -"]
      ++ builtins.concatLists (map
        (share: map (user: "d ${share.target user} 0700 ${user} ${user} -") users)
        cifsShares));
}
